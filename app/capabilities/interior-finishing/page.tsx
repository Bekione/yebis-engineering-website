...
const SPECS = [
  { label: "Surface Tolerance", value: "±1.5mm", detail: "Laser datum control across all planes" },
  { label: "Gypsum Grade", value: "Q4", detail: "European DIN EN 13914 finish standard" },
  { label: "Tile Calibration", value: "±0.3mm", detail: "Large-format porcelain rectified edge" },
  { label: "Acoustic Rating", value: "NRC 0.85", detail: "Perforated ceiling absorption coefficient" },
  { label: "Epoxy Thickness", value: "3mm", detail: "Self-leveling polyurethane floor coat" },
  { label: "Wall Systems", value: "12+", detail: "Microcement, cladding, and paint types" },
];
...
const SERVICES = [
  {
    ...
    image: IMG.cleanroom,
    specs: ["Moisture-Resistant MR Board", "Fire-Rated Type X Assemblies", "Acoustic Perforated Panels", "Hidden LED Integration Channels"],
  },
  {
    ...
    image: IMG.lobby,
    specs: ["1200×600mm Large Format Install", "Polished Terrazzo In-Situ Pour", "Self-Leveling Screed Prep", "Waterproof Membrane Integration"],
  },
  {
    ...
    image: IMG.boardroom,
    specs: ["Venetian Microcement Application", "Timber Slat Acoustic Cladding", "Washable Anti-Bacterial Paint", "Feature Wall Stone Veneer"],
  },
  {
    ...
    specs: ["STC 55 Sound-Rated Walls", "Double-Layer Board Systems", "Glass Partition Integration", "Fire-Rated 2HR Assemblies"],
  },
];
      ...
      <div className="w-full bg-surface-container-low border-b border-outline-variant/30 py-space-xs px-6 lg:px-12">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-y-2 text-on-surface-variant font-label-sm text-label-sm uppercase tracking-wider">
          <div className="flex items-center gap-space-sm">
            ...
            <span>SEC_INT-01 // INTERIOR ARCHITECTURE &amp; FINISHING // DIVISION 05</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-space-xl items-start">
            <div className="lg:col-span-7 flex flex-col gap-space-md">
              ...
              <p className="font-body-lg text-body-lg text-primary font-medium leading-relaxed max-w-3xl">
                Flawless interior surfaces engineered with Swiss tolerance — from acoustic multi-tier gypsum ceilings to heavy-traffic epoxy coatings and calibrated large-format porcelain installations.
              </p>
              <p className="font-body-md text-body-md text-on-surface-variant max-w-3xl leading-relaxed">
                Our interior finishing division delivers the final experience that occupants live with every day. We refuse to compromise on tolerances — every surface undergoes laser-datum verification, every ceiling line is checked to ±1.5mm, and every floor tile is mechanically calibrated before installation.
              </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 border border-outline-variant/40 bg-surface divide-y md:divide-y-0 md:divide-x divide-outline-variant/30">
            {SPECS.map((spec) => (
              <div key={spec.label} className="p-space-md flex flex-col gap-space-xs">
                ...
                <span className="font-headline-sm text-headline-sm font-bold text-on-surface">{spec.value}</span>
      ...
      <section className="w-full bg-surface px-6 lg:px-12 py-space-xl">
        <div className="max-w-7xl mx-auto flex flex-col gap-space-xl">
          {SERVICES.map((service, idx) => (
            ...
            >
              <div className={`lg:col-span-5 relative aspect-[4/3] bg-surface-container overflow-hidden border border-outline-variant/40 ${idx % 2 === 1 ? "lg:order-2" : ""}`}>
                <Image
                  src={service.image}
                  alt={service.title}
                  fill
              ...
              <div className={`lg:col-span-7 flex flex-col gap-space-md ${idx % 2 === 1 ? "lg:order-1" : ""}`}>
                <div className="flex items-center gap-space-sm">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-space-lg">
          <div className="flex flex-col gap-space-xs">
            ...
            <p className="font-body-md text-body-md text-inverse-on-surface">
              From raw concrete to occupancy-ready — our finishing team delivers Q4 standard surfaces within your project timeline.
            </p>
          </div>
          <Link
            href="/start-a-project"
