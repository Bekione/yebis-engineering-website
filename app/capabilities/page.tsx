...
const METRICS = [
  { label: "Civil Scope", value: "G+16", unit: "MAX CAP", desc: "Structural RC framing & deep piling" },
  { label: "Fabrication Capacity", value: "1,800", unit: "M²/MO", desc: "Aluminum, glazing & joinery yard" },
  { label: "MEP Compliance", value: "100%", unit: "EBCS", desc: "Ethiopian Building Code Standards" },
  { label: "Defect Tolerance", value: "±1.5", unit: "MM", desc: "Optical & laser datum control", highlight: true },
];
...
const DIVISIONS = [
  {
    ...
    scope: ["Mass Earthworks & Shoring", "C30/C40 Cast Concrete Frame", "Post-Tensioned Slabs", "Site Logistics & Crane Operations"],
  ...
  },
  {
    ...
    scope: ["MEP Clash Detection (Navisworks)", "Structural Detail Fabrication Drawings", "Parametric 3D Volumetric Renders", "As-Built Dossier Compilation"],
  ...
  },
  {
    ...
    scope: ["Main Distribution Boards (MDB)", "Synchronized Generator Plant & ATS", "Structured Cabling & Fiber Backbones", "Earthing & Lightning Protection Grid"],
  ...
  },
  {
    ...
    scope: ["PPR-C & HDPE Water Reticulation", "Acoustic Silent Waste Stacks (uPVC)", "Duplex Booster Pump Systems", "Elastomeric Structural Waterproofing"],
  ...
  },
  {
    ...
    scope: ["Acoustic & Moisture-Resistant Gypsum", "Calibrated Porcelain & Terrazzo", "Polyurethane & Self-Leveling Epoxy", "Microcement & Textural Wall Systems"],
  ...
  },
  {
    ...
    scope: ["Integrated Solid Wood Executive Suites", "Soft-Close Moisture-Shield Kitchens", "Acoustic Timber Slat Wall Paneling", "Solid Hardwood Architectural Doors"],
  ...
  },
  {
    ...
    scope: ["Unitized & Stick Curtain Walls", "Heavy Duty Commercial Sliding Portals", "Tempered Glass & Steel Balustrades", "Structural Steel Mezzanines & Trusses"],
  ...
  },
  {
    ...
    scope: ["Carbon Fiber Structural Strengthening", "Commercial Floor Demolition & Fit-Out", "HVAC & MEP Upgrades In-Situ", "Preventative Maintenance Contracts"],
      ...
      <section className="w-full bg-surface-container-lowest px-6 lg:px-12 py-space-xl">
        <div className="max-w-7xl mx-auto flex flex-col gap-space-lg">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-space-md">
            <div className="flex flex-col gap-space-xs max-w-3xl">
              ...
              <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl pt-space-xs">
                From foundation civil works to bespoke interior joinery. A unified multi-disciplinary engineering firm that eliminates contractor fragmentation across Ethiopia.
              </p>
            </div>
            <div className="flex flex-col items-start md:items-end gap-space-xs bg-surface-container-low p-space-md">
              <span className="font-label-sm text-label-sm text-secondary uppercase">REGULATORY RATING</span>
              <span className="font-label-lg text-label-lg font-bold text-on-surface">GC-1 CLASSIFICATION</span>
              <span className="font-label-sm text-label-sm text-primary">FULL VERTICAL INTEGRATION</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-space-sm pt-space-md">
            {METRICS.map((m) => (
              <div key={m.label} className="bg-surface-container p-space-md flex flex-col justify-between">
                ...
                <div className="flex items-baseline gap-space-xs mt-space-sm">
                  <span className={`font-headline-lg text-[28px] leading-[36px] lg:text-headline-lg font-bold ${m.highlight ? "text-primary" : "text-on-surface"}`}>
                    {m.value}
                  </span>
                  <span className="font-label-sm text-label-sm text-on-surface-variant">{m.unit}</span>
                </div>
                <span className="font-body-sm text-body-sm text-on-surface-variant mt-space-xs">{m.desc}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="w-full bg-surface px-6 lg:px-12 py-space-xl">
        <div className="max-w-7xl mx-auto flex flex-col gap-space-xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-space-lg">
            {DIVISIONS.map((div) => (
              <article key={div.id} className="bg-surface-container-lowest flex flex-col justify-between p-space-lg shadow-sm">
                <div className="flex flex-col gap-space-md">
                  <div className="flex items-center justify-between bg-surface-container-low p-space-sm">
                    ...
                    <span className="font-label-sm text-label-sm text-secondary uppercase hidden sm:inline">{div.grade}</span>
                  </div>
                  ...
                  <h2 className="font-headline-md text-[22px] leading-[30px] lg:text-headline-md text-on-surface uppercase font-bold">
                    {div.title}
                  </h2>
                ...
                <div className="flex items-center justify-between pt-space-md mt-space-md bg-surface-container-low p-space-sm">
                  <span className="font-label-sm text-label-sm text-secondary">{div.datum}</span>
                  {div.href ? (
                    <Link href={div.href} className="font-label-sm text-label-sm text-primary font-bold hover:underline">
                      {div.cta}
                    </Link>
                  ) : (
                    <span className="font-label-sm text-label-sm text-primary font-bold">{div.cta}</span>
                  )}
                </div>
              </article>
            ))}
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-space-lg">
          <div className="flex flex-col gap-space-xs">
            ...
            <p className="font-body-md text-body-md text-inverse-on-surface max-w-xl">
              Our multi-disciplinary team can evaluate your requirements across all eight engineering divisions within 48 business hours.
            </p>
          </div>
          <Link
